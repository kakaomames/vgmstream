...
class CustomAudio extends HTMLElement{
	...
	constructor(){
		super()
		console.log("ACTION", "CustomAudio: コンストラクタ起動");
		this.attachShadow({mode: "open"})
		this.#addStyle()
	}
	async #addStyle(){
		console.log("ACTION", "CustomAudio: スタイル読み込み開始");
		var css = await (await fetch("css/custom-audio.css")).text()
		if(this.shadowRoot.adoptedStyleSheets){
			console.log("INFO", "CustomAudio: adoptedStyleSheetsを使用します");
			var style = new CSSStyleSheet()
			await style.replace(css)
			this.shadowRoot.adoptedStyleSheets = [style]
		}else{
			console.log("INFO", "CustomAudio: styleタグを生成します");
			var style = document.createElement("style")
			style.appendChild(document.createTextNode(css))
			this.shadowRoot.append(style)
		}
		console.log("SUCCESS", "CustomAudio: スタイル適用完了");
	}
	#shown
	#loaded
	#player
	...
	#seek
	#showPlayer(){
		console.log("ACTION", "CustomAudio: プレイヤー表示処理開始");
		this.#shown = true
		...
		if(this.#loaded){
			console.log("INFO", "CustomAudio: ロード済み状態で表示");
		...
		this.#update(true)
		console.log("SUCCESS", "CustomAudio: プレイヤーDOM構築完了");
	}
	#hidePlayer(){
		console.log("ACTION", "CustomAudio: プレイヤー非表示処理");
		this.#shown = false
	...
	connectedCallback(){
		console.log("ACTION", "CustomAudio: DOM接続（connectedCallback）");
		...
		if(!this.paused){
			console.log("INFO", "CustomAudio: 再生中につきタイマー開始");
			this.#updateTimer = setInterval(this.#update.bind(this), 200)
		}
	}
	disconnectedCallback(){
		console.log("ACTION", "CustomAudio: DOM切り離し（disconnectedCallback）");
		if(this.#shown){
			this.#hidePlayer()
		}
	...
	async attributeChangedCallback(name, oldValue, newValue){
		console.log("ACTION", `CustomAudio: 属性変更検知 [${name}] : ${oldValue} -> ${newValue}`);
		switch(name){
			case "controls":
				if(this.isConnected){
					if(newValue === null){
				...
				break
			case "loop":
				console.log("INFO", `CustomAudio: ループ設定変更 [${this.loop}]`);
				if(!this.paused){
					this.play()
				}
				break
			case "src":
					console.log("ACTION", `CustomAudio: ソース読み込み開始 -> ${newValue}`);
					this.#loaded = false
					if(this.#shown){
					...
					try{
						...
						console.log("ERROR", `CustomAudio: ロード失敗 -> ${e.message}`);
						this.#event("error")
						throw e
					}
					if(this.src !== newValue){
						console.log("WARN", "CustomAudio: ロード中にsrcが変更されたため破棄します");
						return
					}
					this.#sound = sound
					this.#currentTime = 0
					...
					console.log("SUCCESS", "CustomAudio: ソースロード完了");
					this.#event("progress")
					this.#event("canplay")
			...
			case "loop-start":
				...
				console.log("INFO", `CustomAudio: ループ開始地点設定 -> ${this.#loopStart}`);
				if(!this.paused){
					this.play()
				}
				break
			case "loop-end":
				...
				console.log("INFO", `CustomAudio: ループ終了地点設定 -> ${this.#loopEnd}`);
				if(!this.paused){
					this.play()
				}
				break
	...
	set currentTime(value){
		console.log("ACTION", `CustomAudio: シーク移動要求 -> ${value}`);
		if(!this.loop){
	...
	#update(full){
		if(this.#shown){
			...
			if(full){
				console.log("INFO", `CustomAudio: 表示更新 (full) [Paused: ${this.paused}]`);
	...
	#unformatTime(timestamp){
		console.log("ACTION", `CustomAudio: 時間フォーマット解除 -> ${timestamp}`);
		var minus = 1
	...
	#onPlay(){
		console.log("ACTION", `CustomAudio: プレイボタン押下 [Current: ${this.paused ? 'Paused' : 'Playing'}]`);
		if(this.paused){
			this.play()
		}else{
			this.pause()
	...
	#soundStarted
	play(){
		console.log("ACTION", "CustomAudio: 再生開始試行");
		if(!this.#sound){
			console.log("WARN", "CustomAudio: サウンドがロードされていないため再生できません");
			return
		}
		if(!this.paused){
			this.pause()
		...
		if(!this.loop && this.currentTime >= this.duration){
			console.log("INFO", "CustomAudio: 再生位置をリセットします");
			this.#currentTime = 0
		}
		this.paused = false
		this.#soundStarted = this.#sound.getTime()
		if(this.loop){
			console.log("INFO", `CustomAudio: ループ再生開始 [Start: ${this.#loopStart}, End: ${this.#loopEnd}]`);
			this.#sound.playLoop(0, false, this.currentTime, this.#loopStart, this.#loopEnd)
		}else{
			console.log("INFO", `CustomAudio: 通常再生開始 [Time: ${this.currentTime}]`);
	...
	pause(){
		console.log("ACTION", "CustomAudio: 一時停止処理");
		if(this.paused){
			return
		}
		this.paused = true
		...
		console.log("INFO", `CustomAudio: 現在の再生保持位置 -> ${this.#currentTime}`);
		this.#update(true)
		clearInterval(this.#updateTimer)
	...
	#onKey(event){
		...
		var capture = false
		console.log("ACTION", `CustomAudio: キー入力検知 [${event.key}]`);
		switch(event.key){
			case " ":
				this.#onPlay()
				capture = true
	...
	#onSeek(event){
		console.log("ACTION", `CustomAudio: シークバー操作 -> ${event.currentTarget.value}`);
		this.currentTime = parseFloat(event.currentTarget.value)
	}
	#onEnded(){
		console.log("EVENT", "CustomAudio: 再生終了");
		this.pause()
		this.#event("ended")
	}
	#event(name){
		console.log("EVENT", `CustomAudio: カスタムイベント発行 [${name}]`);
